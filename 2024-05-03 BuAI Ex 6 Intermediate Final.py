...
def main(h, x):
    ...
    for step in range(steps):
        ...
        if h[x_new] > h[x] or (T!=0 and random.random()<(np.exp(-(h[x]-h[x_new])/T))):                                     # if new score is better...
            x = x_new                                         # accept the new score       
        else:                                                 # if not
           pass    # check if probability is greater than random num (0-1.0) meaning simulated annealing as in exercise 5
